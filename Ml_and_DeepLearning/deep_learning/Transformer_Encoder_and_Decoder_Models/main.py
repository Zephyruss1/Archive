...
class EmbeddingsWithPositionalEncoding(nn.Module):
    def __init__(self, d_model: int, n_vocab: int, max_len: int = 5000):
        super().__init__()
        self.linear = nn.Embedding(n_vocab, d_model)  # Embedding words. e.g: cat --> 5.14
        self.d_model = d_model  # Embedding size.
        self.register_buffer('positional_encodings', get_poistional_encoding(d_model,
                                                                             max_len))  # Blocking optimizer attach embedding to has already embedded word.
...
class EmbeddingsWithLearnedPositionalEncoding(nn.Module):
    def __init__(self, d_model: int, n_vocab: int, max_len: int = 5000):
        ...
        self.positional_encodings = nn.Parameter(torch.zeros(max_len,
                                                             1, d_model), requires_grad=True)
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, *, x: torch.Tensor, mask: torch.Tensor,
                src: torch.Tensor = None, src_mask: torch.Tensor = None):
