...
class DiscriminatorLogitsLoss(Module):
    ...
    def forward(self, logits_true: torch.Tensor, logits_false: torch.Tensor):
        if len(logits_true) > len(self.labels_true):
            self.register_buffer('labels_true', _create_labels(len(logits_true), 1.0 - self.smoothing,
                                                               1.0, logits_true.device), False)
        if len(logits_false) > len(self.label_false):
            self.register_buffer('labels_false', _create_labels(len(logits_false), 0.0, self.smoothing,
                                                                logits_false.device), False)
...
class GeneratorLogitsLoss(Module):
    ...
    def forward(self, logits: torch.Tensor):
        if len(logits) > len(self.fake_labels):
            self.register_buffer('fake_labels', _create_labels(len(logits), 1.0 - self.smoothing, 1.0,
                                                               logits.device), False)
